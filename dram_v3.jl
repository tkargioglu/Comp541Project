...
#=
"""
run for one episode used for one image because episodes for images have different lengths
    return loss
"""
function dramforward(dram::DRAM, images, deterministic::Bool=false)
    ...
    # initializing hidden state of the rnns, (hidden_size, batchsize, numlayers)
    dram.rnnupper.h = Param(convert(atype, randn(rnn_hidden_size, batchsize, 1))) # upper layer may be initialized with context network
